			...
			bool *foundPtr)
{
	 ...
	 * Usage_count is not used in LRU
	 */
	//buf->usage_count = 0;
...
UnpinBuffer(volatile BufferDesc *buf, bool fixOwner, bool normalAccess)
{
	...
	if (PrivateRefCount[b] == 0)
	{
		...
		*/
            ...
            /* 
			 * LRU Change (RaghunathB)
			 * remove usage_count variables
			 * LRU doesn't use it
			 */
                
			if (buf->refcount == 0) // && buf->usage_count == 0) -- usage_count is not required
		...
		/* 
		 * LRU Change (RaghunathB)
		 * Here we need to push all the free buffers to the tail of the queue
		 * Hence there is no need to check if it is immed_free_buffer
		 * Just unconditionally push all free buffers to the tail of the freelist queue
		 */
		 
		//if (immed_free_buffer) -- no need to check,push all free buffers to the tail
		StrategyFreeBuffer(buf, false); // false -> at_head variable in freelist.c is false. Hence, the insertion into the linked list is done at the tail always.
	}
}
...
SyncOneBuffer(int buf_id, bool skip_pinned)
{
	...
	if (skip_pinned &&(bufHdr->refcount != 0))// || bufHdr->usage_count != 0)) LRU Change (RaghunathB) usage_count not required in LRU
	{
		UnlockBufHdr(bufHdr);
		return false;
	}
